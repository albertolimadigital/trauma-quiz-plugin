=== Trauma Quiz ===
Contributors: albertolimadigital
Tags: trauma, quiz, lgpd, mental health, assessment
Requires at least: 6.2
Tested up to: 7.1
Stable tag: 1.3
Requires PHP: 7.0
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

A light questionnaire for assessing trauma symptoms, LGPD-compliant, with personalized feedback and CSV data export.

== Description ==

**Trauma Quiz** is a simple yet powerful tool designed to help users assess trauma symptoms through a gentle questionnaire. The plugin is fully compliant with the Brazilian General Data Protection Law (LGPD), ensuring user privacy and data security.

Key features include:

*   **Gentle Questionnaire:** A user-friendly interface that guides respondents through a series of questions without causing additional stress.
*   **Personalized Feedback:** Provides immediate, customized feedback to users based on their responses.
*   **CSV Data Export:** Allows administrators to easily export all collected data to a CSV file for further analysis.
*   **LGPD Compliance:** Built with privacy in mind, ensuring all data handling follows LGPD requirements.
*   **Customizable:** The 5 core variables are available in the source code, allowing developers to adapt the questionnaire as needed.

This plugin is ideal for therapists, clinics, and mental health professionals looking for a digital tool to assist with initial trauma assessments.

== Installation ==

1. Upload the `trauma-quiz` folder to the `/wp-content/plugins/` directory, or install the plugin directly through the WordPress plugins screen.
2. Activate the plugin through the 'Plugins' screen in WordPress.
3. Navigate to the plugin's settings page to configure your questions and options.
4. Shortcode: [trauma_quiz]  

== Frequently Asked Questions ==

= Is this plugin LGPD compliant? =

Yes. The plugin is designed with privacy and data security as a priority, adhering to the principles of the Brazilian General Data Protection Law (LGPD).

= Can I customize the questions? =

Absolutely. The 5 core variables are available in the source code, and you can modify the questions and options through the plugin's settings panel.

= What format is the data exported in? =

All collected data can be exported as a standard CSV file, which can be opened with any spreadsheet software like Microsoft Excel or Google Sheets.

== Changelog ==

= 1.3 =
*   Initial release.
